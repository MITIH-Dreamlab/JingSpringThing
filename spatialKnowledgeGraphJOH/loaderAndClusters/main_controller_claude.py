...
class Edge:
    ...
    def update_blender_object(self):
        if self.blender_object:
            ...
            # Visualize edge weight with thickness
...
class World:
    ...
    def load_data(self, nodes_data: List[Dict], edges_data: List[Dict]):
        for node_data in nodes_node = Node(**node_data)
            self.add_node(node)
...
def main(markdown_folder: str):
    try:
        nodes, edges = parse_markdown_files(markdown_folder)
        ...
        print(f"Saved node and edge data to JSON files.")
...
if __name__ == '__main__':
    if len(sys.argv) < 2:
        print("Usage: blender --python main_controller_claude.py -- <path_to_markdown_folder>")
        sys.exit(1)
    
    markdown_folder = sys.argv[-1]
    if not os.path.isdir(markdown_folder):
        print(f"Error: {markdown_folder} is not a valid directory")
        sys.exit(1)
