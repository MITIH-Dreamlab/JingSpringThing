...
class Node:
    def __init__(self, data):
        ...
        # Assign random positions initially, scaled appropriately
...
class World:
    ...
    def create_material_for_node(self, link_count):
        # Create a new material with default nodes and colors
        ...
        # Add output and BSDF diffuse shader
        output = nodes.new(type='ShaderNodeOutputMaterial')
        diffuse = nodes.new(type='ShaderNodeBsdfDiffuse')
        
        # Set color based on link count (simple gradient from blue to red)
        ...
        def create_edge_objects(index=0):
            try:
                if index < len(edges_list):
                    ...
                    # Create a cuboid for the edge
                    bpy.ops.mesh.primitive_cube_add(size=1)
                    obj = bpy.context.object
                    edge.blender_object = obj

                    # Ensure the object is properly positioned and scaled between the nodes
                    direction = mathutils.Vector(end_loc) - mathutils.Vector(start_loc)
                    distance = direction.length
                    direction.normalize()
                    quat = direction.to_track_quat('Z', 'Y')
                    ...
                    obj.scale = (EDGE_THICKNESS, EDGE_THICKNESS, distance / 2)

                    # Debug: Print edge creation information
