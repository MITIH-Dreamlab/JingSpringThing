...
NODE_BASE_SIZE = 0.1
EDGE_THICKNESS = 0.02  # Increased for better visibility
POSITION_SCALE = 10.0
SPRING_CONSTANT = 0.01  # Example spring constant
DAMPING_FACTOR = 0.95  # High damping to prevent snapping
...
class Edge:
    ...
    def update_blender_object(self):
        ...
        if self.blender_object:
            start_loc = self.start_node.blender_object.location
            end_loc = self.end_node.blender_object.location
            
            direction = end_loc - start_loc
            length = direction.length
            
            if length > 0:
                midpoint = start_loc.lerp(end_loc, 0.5)  # Use lerp for midpoint
...
def main():
    ...
    try:
        ...
        # *** COMMENTED OUT THE SIMULATION SECTION FOR DEBUGGING ***
        # max_steps = 1000
        # delta_time = 0.05
        # num_iterations = 1  # Use only one iteration for now 
