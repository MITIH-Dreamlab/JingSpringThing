...
SPRING_CONSTANT = 0.01  # Strength of the spring force (currently not used as simulation is commented out)
DAMPING_FACTOR = 0.95  # Damping to reduce oscillations, higher value = more damping 
ENERGY_THRESHOLD = 1e-4  # Threshold for considering the system at equilibrium (not used currently)
...
MAX_HYPERLINK_COUNT = 20  # Maximum expected number of hyperlinks in a node (adjust based on your data)
MAX_EDGE_WEIGHT = 10 # Maximum expected edge weight (adjust based on your data)
...
class Vector3:
    ...
    Provides basic vector operations like addition, subtraction, 
    scalar multiplication, normalization, and conversion to tuples.
    """
...
class Node:
    ...
    Stores information about the node's ID, name, position, size, links,
    content, and associated Blender object.
    Includes methods for applying forces, updating positions based on physics, 
    and calculating size based on file size.
    """
...
class Edge:
    ...
    Stores information about the connected nodes, weight, type of link,
    and the associated Blender cylinder object.
    Includes methods for applying spring forces and updating the visual 
    representation of the edge in Blender.
    """
...
class World:
    ...
    Provides methods for managing nodes and edges, randomizing node positions,
    and creating the visual representation of the graph in Blender.
    """
    def __init__(self):
    ...
    def create_blender_objects(self):
        ...
        Assigns materials to the objects to visualize node size and hyperlink count.
        """
        for node in self.nodes.values():
        ...
        try:
            with open(file_path, 'r', encoding='utf-8') as file:
                ...
                for link in link_types:
                    if link in node_dict and not any(e.start_node == node_dict[link] and e.end_node == node_dict[node_name] for e in edges):
