...
class Edge:
    ...
    Attributes:
      ...
      - weight: Weight of the edge, representing the spring constant
      ...
      - rest_length: The initial distance between the nodes, used for spring calculations
    Methods:
      ...
      - increment_weight: Increase the edge weight (spring constant) by 1.
    """
    def __init__(self, start_node: Node, end_node: Node, weight: float = 1.0,
        ...
        self.weight = weight  # Weight of the edge, representing the spring constant
        ...
        self.rest_length = (self.start_node.size + self.end_node.size)  # Calculate rest length based on initial node sizes

    def apply_spring_force(self):
        """
        Calculate and apply a spring force between the two connected nodes.
        The force is proportional to the distance and the edge weight (spring constant).
        ...
        if distance == 0:
            return 0
        force_magnitude = SPRING_CONSTANT * self.weight * (distance - self.rest_length) # Weight influences force magnitude
    ...
    def increment_weight(self):
        """
        Increase the edge weight (spring constant) by 1.
        Represents adding another mention of the link between nodes.
        """
        self.weight += 1
...
class World:
    ...
    def create_blender_objects(self):
        ...
        for edge in self.edges:
            ...
            material = create_material_for_edge(edge)  # Pass the edge object
...
def parse_markdown_files() -> Tuple[List[Node], List[Edge]]:
    ...
    Identifies links between nodes, creating Edge objects or incrementing 
    existing edge weights to represent link frequency. 
            ...
            with open(file_path, 'r', encoding='utf-8') as file:
                ...
                for link in link_types:
                    if link in node_dict:
                        # Find if an edge already exists between these nodes
                        existing_edge = next((e for e in edges if e.start_node == node_dict[node_name] and e.end_node == node_dict[link]), None)

                        if existing_edge:
                            existing_edge.increment_weight() # Increase weight if edge exists
                        else:
...
# -----------------------------------------------------------------------------
# Functions for Physics Calculation and Simulation
# -----------------------------------------------------------------------------
...
def create_material_for_edge(edge: Edge): 
    """
    Create a new material with a color based on the composite edge weight.
    Uses a color ramp to interpolate between colors, representing connection strength.
    """
    mat = bpy.data.materials.new(name=f"EdgeMaterial_{edge.start_node.name}_{edge.end_node.name}")
    mat.use_nodes = True
    nodes = mat.node_tree.nodes
    links = mat.node_tree.links
    ...
    # Set color ramp control points (adjust gradient as needed)
    color_ramp.color_ramp.elements[0].color = (0, 0, 1, 1)  # Blue (weak connection)
    color_ramp.color_ramp.elements[1].color = (1, 0, 0, 1)  # Red (strong connection)

    links.new(color_ramp.outputs['Color'], diffuse.inputs['Color'])
    links.new(diffuse.outputs['BSDF'], output.inputs['Surface'])

    # Use the edge weight to determine the color
    normalized_weight = edge.weight / MAX_EDGE_WEIGHT
    color_ramp.inputs['Fac'].default_value = normalized_weight 
...
def main():
    ...
    and runs the physics simulation.
    """
    try:
