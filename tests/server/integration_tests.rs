...
async fn test_end_to_end_workflow() {
    ...
    let processed_content = PerplexityService::process_file(file_content.clone()).await.expect("File processing failed");
...
async fn test_graph_update_workflow() {
    ...
    let processed_initial = PerplexityService::process_file(initial_content.clone()).await.expect("Initial file processing failed");
    ...
    let processed_updated = PerplexityService::process_file(updated_content.clone()).await.expect("Updated file processing failed");
